            ...
            <div className="flex justify-center h-screen">
                ...
                <div className="bg-white 2xl:h-144 2xl:w-288 xl:h-120 xl:w-240 self-center rounded-lg drop-shadow-all">
                    <div className="flex 2xl:h-16 xl:h-14 w-full justify-center items-center 2xl:mt-8 xl:mt-6">
                        ...
                        <div className="bg-verde-padrao border-4 border-verde-padrao rounded-full 2xl:h-12 2xl:w-12 xl:h-10 xl:w-10 " />
                        <div className="bg-verde-padrao h-1 2xl:w-14 xl:w-10" />
                    ...
                    <div className="flex flex-row 2xl:justify-center">
                        ...
                        <div className="flex flex-col justify-around mt-8 gap-8 h-40 w-120 pr-12 pl-12  border-r-2  after:border-slate-300 border-slate-300">
                            ...
                            <div className="relative">
                                <div className="relative inline-block w-full">
                                    <select ref={area_input} onBlur={()=> {validarArea()}} onChange={(e) => { setSelectedArea(e.target.value) }} className={`cursor-pointer block appearance-none w-full text-xl font-bold h-14 bg-white border ${validacaoArea === 1 ? `border-red-500` : `border-cinza-claro-1`} hover:border-gray-500 px-4 py-2 pr-8 rounded shadow leading-tight focus:outline-none focus:shadow-outline`}>
                                        <option key="0" value="0" >Selecione sua area de atuação:</option>
                            ...
                            <div className="relative">
                                ...
                                {dropDown && <>
                                    <button onClick={() => { setdropDown(false) }} className="z-40 fixed h-screen w-screen top-0 left-0 right-0 bottom-0 cursor-default" />
                                    <div className={`z-50 absolute w-full bg-white border ${validacaoServico === 1 ? `border-red-500` : `border-cinza-claro-1`} hover:border-gray-500 px-4 py-2 pr-8 rounded shadow leading-tight focus:outline-none focus:shadow-outline`}>
                        ...
                        <div className="flex flex-col justify-around mt-8 gap-8 h-40 w-120 pr-12 pl-12  border-l-2   after:border-slate-300 border-slate-300">
                            ...
                            <div className="relative inline-block w-full">
                                <select onBlur={()=>{validarEnsinar()}} ref={ensinar_input} className={`cursor-pointer block appearance-none w-full text-xl font-bold h-14 bg-white border ${validacaoEnsinar === 1 ? `border-red-500` : `border-cinza-claro-1`} hover:border-gray-500 px-4 py-2 pr-8 rounded shadow leading-tight focus:outline-none focus:shadow-outline`}>
                    ...
                    <div className="flex justify-center flex-col items-center w-full">
                        ...
                        <div className="flex justify-center w-full 2xl:w-[82%] mt-7">
                            <div className={`relative w-20 h-8 border-4 ${validacaoValorMin === 1 ? `border-red-500` : `border-verde-padrao`} rounded-md text-xl font-medium`}>
                                <span>
                                    R$ <input onBlur={()=>{validarMin()}} className={`min-max-inputs w-16 ml-1 appearance-none focus:ring-0 focus:outline-none focus:border-none bg-transparent absolute`} value={range[0]} type="number" ref={min} onChange={() => { setRange([min.current.value, ...range.slice(1)]) }} />
                                </span>
                            </div>
                            ...
                            <div className={`w-20 h-8 border-4 ${validacaoValorMax === 1 ? `border-red-500` : `border-verde-padrao`} rounded-md text-xl font-medium`}>
                                <span>
                                    R$<input onBlur={()=>{validarMax()}} className={`min-max-inputs w-16 ml-1 appearance-none focus:ring-0 focus:outline-none focus:border-none bg-transparent absolute`} value={range[1]} type="number" ref={max} onChange={() => { setRange([range[0], max.current.value, ...range.slice(2)]) }} />
                                </span>
                            </div>
                        </div>
