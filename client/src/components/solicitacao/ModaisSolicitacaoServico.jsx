...
export default function ModaisSolicitacaoServico(props) {
    ...
    useEffect(() => {
        axios.get(`solicitacao/servicos/${localStorage.getItem("ID_CADASTRANTE")}`) // pegar a info certa de ID do contratante
            .then((response) => {
                            ...
                            {mapServico ? servicos.map((data, index) => (
                                    <div key={index} className="block min-h-6">
                                        <label className='flex items-center'>
                                            <input  className="bg-verde-escuro-1 w-[60px]" defaultChecked={data.checked} onChange={(e) => { alterarChecked(index, e.target.checked) }} id={data.item.id} type="checkbox" />
